...
class LandscapeTypeParams:
    ...
    def __post_init__(self) -> None:
        ...
        if abs(w_sum - 1.0) > 1e-9:
            raise ValueError(f"Weights must sum to 1.0, got {w_sum:.12f}")
...
APPLIANCE_EFFICIENCY_PARAMS: dict[str, ApplianceEfficiencyParams] = {
    "north": ApplianceEfficiencyParams(alpha=9.36, beta=3.64, label="North Hemisphere"),
    "south": ApplianceEfficiencyParams(alpha=4.22, beta=4.55, label="South Hemisphere"),
}
