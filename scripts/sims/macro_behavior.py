...
class MacroBehavioralSimulator:
    def __init__(
        ...
        self._logger.info("Macro-Behavioral Simulator Initialized. Days: %d", self.n_days)

    def _generate_watering_ban(self, cdd: np.ndarray, season_label: np.ndarray) -> np.ndarray:
        """
        ...
        for t in range(self.n_days):
            ...
            if (
                cdd[t] >= self.params.ban_cdd_trigger
                and season_label[t] in ("spring", "summer")
                and t > ban_expiry_day
            ):
                # Initiate ban for 7 to 21 days
                ban_duration = self.rng.integers(
                    self.params.ban_duration_min, self.params.ban_duration_max + 1
                )
    ...
    def generate_features(
        ...
        ban_active = self._generate_watering_ban(
            cdd=consecutive_dry_days, season_label=season_label
        )
        ...
        for cluster_name, usage_arr in cluster_means_dict.items():
            ...
            # Holiday multiplier
            base_usage = np.where(
                holidays == 1, base_usage * self.params.holiday_multiplier, base_usage
            )

            # Pricing penalty
            base_usage = np.where(
                pricing_regime == 1, base_usage * self.params.pricing_tier_1_multiplier, base_usage
            )
            base_usage = np.where(
                pricing_regime == 2, base_usage * self.params.pricing_tier_2_multiplier, base_usage
            )

            # Watering ban compliance
            if "outdoor" in cluster_name.lower() or "landscape" in cluster_name.lower():
                base_usage = np.where(
                    ban_active == 1, base_usage * self.params.ban_compliance_outdoor, base_usage
                )
            else:
                base_usage = np.where(
                    ban_active == 1, base_usage * self.params.ban_compliance_base, base_usage
                )
