...
from params import APPLIANCE_EFFICIENCY_PARAMS, LANDSCAPE_TYPE_PARAMS, OCCUPANCY_PARAMS
...
class HouseholdSimulator:
    ...
    def generate_household_ids(self) -> np.ndarray:
        # generates uuid v7
        self._logger.info("Generating %d household UUIDs (v7)....", self._population_size)
        ...
        self._logger.info("Household ID Stats | unique=%d | sample='%s'", len(id_set), result[0])
    ...
    def generate_occupancy_count(
        ...
        rate = self._rng.gamma(
            shape=params.r, scale=(1.0 - params.p) / params.p, size=self._population_size
        )
    ...
    def generate_appliance_efficiency_score(
        ...
        self._logger.info(
            ...
            params.lower_bound,
            params.upper_bound,
            params.theoretical_mean,
            params.theoretical_mode,
        )

        # 1. vectorized beta draw
        raw_scores = self._rng.beta(a=params.alpha, b=params.beta, size=self._population_size)
    ...
    def generate_landscape_type(self, hemisphere: Literal["north", "south"]) -> np.ndarray:
        ...
        try:
            ...
            self._logger.warning(
                "RNG weight normalization failed (%s).Applying fallback: all -> '%s'.",
                e,
                params.fallback_category,
            )
            result = np.full(self._population_size, params.fallback_category, dtype=object)

        self._logger.info(
            "Landscape Type Stats | unique=%d | mode='%s' | n=%d",
            len(np.unique(result)),
            pl.Series(result).mode().item(0),
            self._population_size,
        )
    ...
    def generate_daily_water_usage_liters(
        ...
        total_usage_liters = np.empty(
            (self._population_size, self._simulation_days), dtype=np.float32
        )
        ...
        temporal_multiplier = np.where(is_weekend == 1, weekend_multiplier, np.float32(1.0))[
            np.newaxis, :
        ]

        efficiency_penalty = 1.0 + (1.0 - appliance_efficiency_2d)
        indoor_baseline = (
            physiological_intake + (occupancy_2d * temporal_multiplier * per_capita_baseline)
        ) * efficiency_penalty
        ...
        landscape_coeffs = np.array(
            [landscape_map.get(lt, 0.5) for lt in landscape_type], dtype=np.float32
        )[:, np.newaxis]

        outdoor_demand = env_deficit * yard_area_sqm * landscape_coeffs

        # axiomatic noise injection
        human_noise = self._rng.normal(
            loc=0.0, scale=4.5, size=(self._population_size, self._simulation_days)
        ).astype(np.float32)
        ...
        macro_grid_noise = self._rng.normal(loc=0.0, scale=15.0, size=self._simulation_days).astype(
            np.float32
        )
