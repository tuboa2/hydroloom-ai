...
from params import HEMISPHERE_TEMPERATURE_PARAMS, SEASON_BOUNDARIES_NORTH, SOUTH_PHASE_SHIFT
...
class EnvironmentalSimulator:
    ...
    def _generate_base_seasonal_curve(
        ...
        for t in range(1, total_months):
            climate_shifts_monthly[t] = (
                params.climate_rho * climate_shifts_monthly[t - 1]
            ) + shocks[t]

        # Interpolate monthly shifts to daily resolution
        month_indices = np.arange(day_index.size, dtype=np.float32) / 30.416
        delta_climate: np.ndarray = np.interp(
            month_indices, np.arange(total_months), climate_shifts_monthly
        ).astype(np.float32)
        # angular frequency
        ...
        if hemisphere == "north":
            base_curve = (params.annual_mean + delta_climate) - params.amplitude * np.cos(
                angular_curve
            )
        else:
            base_curve = (params.annual_mean + delta_climate) + params.amplitude * np.cos(
    ...
    def generate_daily_max_temp(
        ...
        *,
        ...
        ou_noise = self._generate_ou_noise(theta=params.ou_theta, mu=0.0, sigma=params.ou_sigma)
    ...
    def generate_season_labels(
        self, day_index: np.ndarray, *, hemisphere: Literal["north", "south"]
    ) -> np.ndarray:
        # vectorized season assignment O(n)
        assert day_index.dtype == np.int32, f"day_index must be int32, got {day_index.dtype}"
        assert day_index.shape == (1825,), f"Expected shape (1825,), got {day_index.shape}"
        ...
        assert np.all(labels != ""), "Unlabeled day detected - Season boundary gaps"

        return labels[day_of_year]

    # new: generate temporal framework
    def generate_temporal_framework(
        self, config: GlobalInitializer, *, hemisphere: Literal["north", "south"]
    ) -> dict[str, np.ndarray]:
        ...
        year_index: np.ndarray = (day_index // days_per_year).astype(np.int8)

        # feature 2.3: is_weekend
        is_weekend: np.ndarray = ((day_index % 7) >= 5).astype(np.int8)

        # feature 2.4: season_label
        season_label: np.ndarray = self.generate_season_labels(day_index, hemisphere=hemisphere)
