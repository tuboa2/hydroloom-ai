...
class BehavioralFeatureExtractor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: dict) -> pd.DataFrame:
        ...
        features[:, 2] = np.where(mean_water_usage > 0, std_water_usage / mean_water_usage, 0.0)
        ...
        features[:, 3] = np.where(wet_mean > 0, dry_mean / wet_mean, 1.0)
        ...
        features[:, 5] = np.clip(
            np.where(weekday_mean > 0, weekend_mean / weekday_mean, 1.0), 0.75, None
        )
        ...
        features[:, 6] = np.array(
            [landscape_map.get(lt, 0.5) for lt in landscape], dtype=np.float32
        )
        ...
        features[:, 7] = np.where(winter_mean > 0, summer_mean / winter_mean, 1.0)

        # feature 9: drought responsiveness index
        rolling_rain = np.convolve(self._dry_day_mask.astype(np.float32), np.ones(7), mode="same")
        drought_days = rolling_rain >= 6.0
        if drought_days.sum() > 0:
            drought_usage = water_usage[:, drought_days].mean(axis=1)
            features[:, 8] = np.where(mean_water_usage > 0, drought_usage / mean_water_usage, 1.0)
        else:
            features[:, 8] = 1.0
        ...
        features[:, 9] = np.where(p95 > 0, p25 / p95, 1.0)
...
north_pipeline = Pipeline(
    [
        ...
        (
            "scale",
            clone(scaling_pipeline),
        ),  # fixed: clone scaling pipeline to isolate weights for each hemisphere
    ]
)
