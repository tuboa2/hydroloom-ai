...
data_dir = parent_dir / "services/behavior_clustering/data"
data_dir.mkdir(parents=True, exist_ok=True)
model_dir = parent_dir / "services/behavior_clustering/model"
...
def main():
    ...
    north_clusters = north_df.with_columns(pl.Series("cluster_id", north_labels).cast(pl.Utf8))

    print("Clustering South Hemisphere...")
    south_labels = kmeans.fit_predict(south_df.to_numpy())
    south_clusters = south_df.with_columns(pl.Series("cluster_id", south_labels).cast(pl.Utf8))
