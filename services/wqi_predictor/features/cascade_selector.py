...
class FeatureSelectionCascade:
    ...
    def lasso_screening(self, X_train: pl.DataFrame, y_train: pl.Series) -> dict[str, bool]:
        ...
        return {col: abs(coef) < 1e-6 for col, coef in zip(X_train.columns, lasso.coef_)}

    def permutation_importance(
        self, X_train: pl.DataFrame, y_train: pl.Series, X_val: pl.DataFrame, y_val: pl.Series
    ) -> tuple[RandomForestRegressor, dict[str, bool], pl.Series]:
        ...
        perm_zero_flags = {col: imp <= 0 for col, imp in zip(X_train.columns, importance_means)}
        return rf_baseline, perm_zero_flags, importance_means

    def treeshap(
        self, rf_baseline: RandomForestRegressor, X_val: pl.DataFrame, y_std: float
    ) -> dict[str, bool]:
