...
def validate_schema(dataframe: pl.DataFrame, hemisphere: str) -> None:
    ...
    if unknown:
        logger.error(
            "%s: unexpected unknown columns: %s", hemisphere, sorted(unknown)
        )
        raise DataValidationError(
            f"{hemisphere}: unexpected unknown columns detected: {sorted(unknown)}"
        )

    logger.debug("%s: schema validation passed with %d droppable extra columns.", hemisphere, len(droppable))
...
def validate_no_nulls(dataframe: pl.DataFrame) -> None:
    ...
    if columns_with_nulls:
        logger.error("Null values detected in %d columns: %s", len(columns_with_nulls), columns_with_nulls)
        raise DataValidationError(
...
def validate_hemisphere_constant(dataframe: pl.DataFrame, hemisphere: str) -> None:
    ...
    if len(values) != 1 or values[0] != hemisphere.lower():
        logger.error(
            "Hemisphere column expected '%s', found: %s", hemisphere, values
        )
        raise DataValidationError(
